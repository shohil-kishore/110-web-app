...
var ytPlayerVars = {'autoplay': 1, 'controls': 0, 'origin': 'http://www.localhost.com:8888'}; //player vars
...
window.onEngine = function() {
    ...
    Engine.internal().addRPCHost("u1.objecthub.io");
    ...
    Databases.at(session.link(server.root())).create().get(function(db) {
		
        seed = db.root().append("cases", "./caseData");
        types = db.root().append("types", "./types");
		
        client.commit().get(function commitCallback() {
			//console.log('write case d '+(new Date().getTime() - start));
            window.gamedata.writeCaseData(session, seed, function() {
                ...
                var caseNodes = seed.selectAll().select(t.brandName);	
                ...
                //console.log('render strat q '+(new Date().getTime() - start));
                
                // get gamedate of current case
                window.renderStrategyQuestion(session, session.link(getCaseUri()), function() {
...
window.renderStrategyQuestion = function(session, caseNode, onSuccess) {
	...
	*/
    ...
    Flow.parallel([brandName, brandImage, brandVideo, industry, vision, correctStrategy, competitiveScope, costStrategy, correctIndustryStructure, threatOfSubsitutes, supplierPower, rivarly, buyerPower, newEntrants, correctValueChainActivites, valueChainJustification, processName, importantProcess, processSteps, systems, orgUnits]).get(function() {
        ...
        //console.log('create case d '+(new Date().getTime() - start));
